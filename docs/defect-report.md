...
| D4 | 対応済み。要約に `work_completed` を出し、`tests` と `work_completed` が実行者申告であることを明示した。 | `7dba34f` fix: claude経路の構造化出力を有効にし要約が実行者の本文を運ぶようにする | 自己申告自体は観測事実ではないため、必要に応じてログや差分との照合が必要である。 |
