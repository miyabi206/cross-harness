...
1. `runner.py:783` の write 限定を撤廃し、失敗コマンドによる降格を write ロールにも適用する。
...
今回の引き金は、D3 を確認するために存在しないファイルを `Read` した操作だった。**D3 を実証した行為そのものが run を失敗扱いにした。**
